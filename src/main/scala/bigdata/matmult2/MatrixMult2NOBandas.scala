...
object MatrixMult2NOBandas {
   ...
   */
    ...
    val mat1_step1 = mat1.zipWithIndex().map{ case (k, v) => (v+1, k)} 
    val mat2_step1 = mat2.zipWithIndex().map{ case (k, v) => (v+1, k)} 

    if(isDebug) mat1_step1.foreach(println)
    if(isDebug) mat2_step1.foreach(println)
    ...
    val mat1_step2 = mat1_step1.flatMap{ case (k, v) =>  getIndexColVar(k.toString(), ncol.toInt, v) }  
    if(isDebug) mat1_step2.foreach(println)
    
    val mat2_step2 = mat2_step1.flatMap{ case (k, v) =>  getIndexRowVar(k.toString(), nrows.toInt, v) }  
    if(isDebug) mat2_step2.foreach(println)
    ...
    val matJoin = mat1_step2.join(mat2_step2)  
    println("matJoin:")
    if(isDebug) matJoin.foreach(println)
    
    //Realiza la multiplicacion y suma de la fila por la columna
    val result =  matJoin.map{ case (k, v) => (k -> multiply(v)) }
